            ...
            await (await _schedulerFactory.GetScheduler()).ScheduleJob(JobBuilder<TestJob>.Create().UsingJobData(b =>
                {
                    ...
                    b.Force = true;
                    b.Cancel = true;
                }).WithIdentity("Test_Continued", context.JobDetail.Key.Group).Build(),
                TriggerBuilder.Create().WithIdentity("Test_Continued", context.JobDetail.Key.Group).StartNow().Build());
    }
}